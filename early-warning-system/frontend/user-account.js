...
(function () {
    ...
    function _renderUserProfileHtml(j) {
        const cov =
            Array.isArray(j.cities) && j.cities.length ? j.cities.join(', ') : j.city || '—';
        ...
        const rows = [
            ...
            ['Partner approval', j.approval_status != null ? String(j.approval_status) : '—'],
            ['Account active', j.active === false ? 'Archived — contact support' : 'Active'],
    ...
    function renderFacilitiesAndPrefs(j) {
        ...
        const sites = siteLabelsFromProfile(j);
        const thMap =
            j.facility_site_pm25_thresholds && typeof j.facility_site_pm25_thresholds === 'object'
  ...
  <div class="action-buttons" style="display:flex;flex-wrap:wrap;gap:8px;">
    ...
    function showChannelForm(visible, j) {
        ...
        if (visible && j) {
            ...
            const ch = Array.isArray(j.preferred_channels) ? j.preferred_channels.map(String) : [];
    ...
    async function refreshUserRegistrationProfile() {
        ...
        wrap.innerHTML = '<div class="user-profile-empty">Loading your registration…</div>';
        try {
    ...
    function syncFacilityReportingChrome() {
        ...
        loginBlock.style.display = 'block';
        banner.textContent = 'Not signed in — use password or OTP, then manage facilities below.';
    }
    ...
    async function refreshFacilityAuditLog() {
        ...
        try {
            ...
            wrap.innerHTML = rows
                .slice(0, 50)
                .map((row) => {
                    const ts = row.timestamp ? String(row.timestamp).replace('T', ' ') : '';
                    const at = escapeAttr(String(row.action_type || '').replace(/</g, ''));
    ...
    async function refreshNotificationInbox() {
        ...
        try {
            ...
            if (!r.ok) {
                wrap.textContent =
                    typeof j.detail === 'string' ? j.detail : 'Could not load notifications.';
                return;
            }
            const rows = j.entries || [];
            if (!rows.length) {
            ...
            wrap.innerHTML =
                ...
                rows
                    .map((row) => {
                        ...
                        const meta = [row.city, row.alert_level].filter(Boolean).join(' · ') || '—';
                        const body =
                            row.message != null
    ...
    function startEditSharedContact(id) {
        ...
        if (pan && pan.scrollIntoView) pan.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
    ...
    async function refreshSharedContacts() {
        ...
        try {
            ...
            const daily = lim.notify_recipients_daily_max != null ? lim.notify_recipients_daily_max : '—';
            const sent = lim.notify_recipients_sent_today != null ? lim.notify_recipients_sent_today : 0;
    ...
    async function submitSharedContactForm() {
        ...
        try {
            if (editId) {
                const patch = /** @type {Record<string, unknown>} */ ({ display_name: name, channel: ch });
                if (ch === 'email') patch.email = email;
                else patch.phone_e164 = phone;
                const r = await fetch(`${API_BASE}/auth/shared-contacts/${encodeURIComponent(editId)}`, {
                    method: 'PATCH',
                    headers: {
    ...
    async function submitSharedNotify() {
        ...
        if (!tok || !tx) return;
        const picks = Array.from(document.querySelectorAll('.shared-contact-pick:checked'));
        const ids = picks.map((el) => String(el.value || '').trim()).filter(Boolean);
        ...
        try {
            ...
            if (r.status === 429) {
                if (msg)
                    msg.textContent =
                        typeof j.detail === 'string' ? j.detail : 'Daily send limit reached.';
                await refreshSharedContacts();
                return;
            }
            if (!r.ok) {
    ...
    async function facilitySubmitRegistrationVerifyCode() {
        ...
        if (!email || !code) {
            if (msg) {
                ...
                msg.textContent = 'Enter your registration email above and the verification code.';
            }
            return;
        }
        if (msg) {
        ...
        try {
            ...
            if (msg) {
                ...
                msg.textContent = j.message || 'Verified — use Sign in with your password.';
            }
            if (banner) {
                banner.textContent =
    ...
    async function facilitySendLoginCode(evt) {
        ...
        try {
            ...
            if (!r.ok && r.status !== 429) {
                banner.textContent = `Could not request OTP (HTTP ${r.status}). ${typeof j.detail === 'string' ? j.detail : ''}`.trim();
                return;
            }
    ...
    async function postFacilityOperationalAction(actionKey, evt, facilitySite) {
        ...
        if (sites.length > 1 && !facility_site) {
            if (banner) banner.textContent = 'Choose a facility card above — which site are you reporting for?';
            return;
        }
        try {
            const payload = {
            ...
            if (r.status === 401 || r.status === 403) {
                ...
                if (banner)
                    banner.textContent =
                        (typeof j.detail === 'string' ? j.detail : 'Session expired') + ' — sign in again.';
                return;
            }
            if (!r.ok) {
                const det =
            ...
            if (banner) banner.textContent = err && err.message ? err.message : 'Action network error.';
            console.warn(err);
        }
    }
    ...
    function setupUserAccountListeners() {
        ...
        if (fcSend) fcSend.addEventListener('click', (ev) => facilitySendLoginCode(ev));
        const fcEx = document.getElementById('facilityExchangeBtn');
        if (fcEx) fcEx.addEventListener('click', (ev) => facilityExchangeLoginCode(ev));
        const fcOut = document.getElementById('facilityLogoutBtn');
        if (fcOut) fcOut.addEventListener('click', () => facilitySignOutReporting());
        const regV = document.getElementById('regSignupVerifyBtn');
        if (regV) regV.addEventListener('click', () => facilitySubmitRegistrationVerifyCode());
        const upRf = document.getElementById('userProfileRefreshBtn');
        if (upRf) upRf.addEventListener('click', () => refreshUserRegistrationProfile());
        const nInb = document.getElementById('notificationInboxRefreshBtn');
        if (nInb) nInb.addEventListener('click', () => refreshNotificationInbox().catch(() => {}));
        const addF = document.getElementById('addFacilityBtn');
        if (addF) addF.addEventListener('click', () => onAddFacility().catch(() => {}));
        const saveTh = document.getElementById('saveThresholdsBtn');
        if (saveTh) saveTh.addEventListener('click', () => onSaveThresholds().catch(() => {}));
        const saveCh = document.getElementById('channelPrefsSaveBtn');
        if (saveCh) saveCh.addEventListener('click', () => onSaveChannels().catch(() => {}));

        const sc = document.getElementById('sharedContactChannel');
        if (sc) sc.addEventListener('change', () => syncSharedContactChannelFields());
        syncSharedContactChannelFields();
        const ss = document.getElementById('sharedContactSubmitBtn');
        if (ss) ss.addEventListener('click', () => submitSharedContactForm().catch(() => {}));
        ...
        if (sct) {
            sct.addEventListener('click', (evt) => {
                const ed = evt.target && evt.target.closest ? evt.target.closest('.shared-contact-edit') : null;
                const del = evt.target && evt.target.closest ? evt.target.closest('.shared-contact-del') : null;
                if (ed) {
        ...
        if (sn) sn.addEventListener('click', () => submitSharedNotify().catch(() => {}));

        const root = document.getElementById('facilitiesRoot');
        if (root) {
            root.addEventListener('click', (evt) => {
                const tgt = evt.target && evt.target.closest ? evt.target.closest('.fac-action-btn') : null;
                if (!tgt) return;
