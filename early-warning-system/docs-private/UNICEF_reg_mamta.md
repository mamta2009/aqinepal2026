...
- **Canonical spec:** `early-warning-system/docs/IMPLEMENTATION_SNAPSHOT.md` (resolver: WeatherAPI direct → WAQI → Rapid for `/api/air-quality/current`; provenance field **`deployment_role`**; OpenWeather routes are supplementary; **`DEPLOYMENT_MODE`** default **REGIONAL**).
